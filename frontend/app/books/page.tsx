...
const BookIndex = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/books")// Book全権取得のRailsAPIを叩いている
    .then((res) => res.json())
  ...
  const deleteBook = async (id: number) => {
    await axios.delete(`http://localhost:3000/books/${id}`); // HTTP DELETEリクエストを送信
    setBooks(books.filter((book) => book.id !== id)); // 削除された本を除いた新しい配列を設定(フロントエンドの状態を更新)
  };
